...
public class Settings {
    ...
    private static final String SERVERADDRESS = "SERVERIP";
    ...
    public synchronized static String loadServerAddress(Context context) {
        if (serverAddress == null) {
            File serverAddressFile = new File(context.getFilesDir(), SERVERADDRESS);
            try {
                if (serverAddressFile.exists())
                    serverAddress = readFile(serverAddressFile);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }
    ...
    public synchronized static String saveServerAddress(Context context, String serverIP) {
        File serverAddressFile = new File(context.getFilesDir(), SERVERADDRESS);
        try {
            writeFile(serverAddressFile, serverIP);
